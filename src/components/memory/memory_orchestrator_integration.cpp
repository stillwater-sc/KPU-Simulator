...
// MemoryOrchestratorBlockMoverAdapter Implementation
MemoryOrchestratorBlockMoverAdapter::MemoryOrchestratorBlockMoverAdapter(MemoryOrchestrator* orchestrator, size_t adapter_id)
...
void MemoryOrchestratorBlockMoverAdapter::orchestrated_block_transfer(
    ...
    builder.prefetch(dst_orchestrator_bank, src_offset, dst_offset, transfer_size);

    if (completion_callback) {
        builder.compute(dst_orchestrator_bank, completion_callback);
    }

    builder.execute_on(*orchestrator);
}

void MemoryOrchestratorBlockMoverAdapter::double_buffered_transfer(
    size_t src_l3_tile_id, Address src_offset,
    ...
    orchestrator->orchestrate_double_buffer(primary_bank, secondary_bank, src_offset, transfer_size);
}

void MemoryOrchestratorBlockMoverAdapter::pipelined_transfer_compute(
    size_t src_l3_tile_id, Address src_offset,
    ...
    orchestrator->orchestrate_pipeline_stage(input_bank, output_bank, compute_operation);
}

bool MemoryOrchestratorBlockMoverAdapter::is_busy() const {
    return orchestrator ? orchestrator->is_busy() : false;
}

void MemoryOrchestratorBlockMoverAdapter::reset() {
    if (orchestrator) {
        orchestrator->reset();
    }
}

// MemoryOrchestratorStreamerAdapter Implementation
MemoryOrchestratorStreamerAdapter::MemoryOrchestratorStreamerAdapter(MemoryOrchestrator* orchestrator, size_t adapter_id)
    : orchestrator(orchestrator), adapter_id(adapter_id) {
    if (orchestrator) {
...
void MemoryOrchestratorStreamerAdapter::start_eddo_stream(const EDDOStreamConfig& config) {
    if (!orchestrator) return;
    ...
    if (config.enable_prefetch_pipelining) {
        ...
        for (size_t depth = 0; depth < config.prefetch_depth; ++depth) {
            ...
            builder.prefetch(config.orchestrator_bank_id, prefetch_addr, 0, prefetch_size);
        }
    } else {
        // Single prefetch stage
        builder.prefetch(config.orchestrator_bank_id, config.orchestrator_base_addr, 0, stream_size);
    }

    // Add compute phase for streaming
    builder.compute(config.orchestrator_bank_id, [config]() {
    ...
    if (config.direction == Streamer::StreamDirection::L1_TO_L2) {
        builder.writeback(config.orchestrator_bank_id, 0, config.l1_base_addr, stream_size);
    }

    builder.execute_on(*orchestrator);
}

void MemoryOrchestratorStreamerAdapter::orchestrated_systolic_stream(
...
void MemoryOrchestratorStreamerAdapter::multi_phase_stream(const std::vector<EDDOStreamConfig>& phases) {
    for (const auto& phase : phases) {
        start_eddo_stream(phase);
    }
}

bool MemoryOrchestratorStreamerAdapter::is_streaming() const {
    return orchestrator ? orchestrator->is_busy() : false;
}

void MemoryOrchestratorStreamerAdapter::abort_stream() {
    if (orchestrator) {
        orchestrator->abort_pending_commands();
    }
}

void MemoryOrchestratorStreamerAdapter::reset() {
    if (orchestrator) {
        orchestrator->reset();
    }
}
...
EDDOMatrixOrchestrator::EDDOMatrixOrchestrator(MemoryOrchestrator* orchestrator, size_t orchestrator_id)
            ...
            for (size_t tile_k = 0; tile_k < num_tiles_k; ++tile_k) {
                ...
                workflow.compute(bank_c, [=, &config, this]() {
                    if (config.compute_kernel) {
                        ...
                        orchestrator->read(bank_a, 0, tile_a_data.data(), tile_a_size);
                        orchestrator->read(bank_b, 0, tile_b_data.data(), tile_b_size);
                        ...
                        orchestrator->write(bank_c, 0, tile_c_data.data(), tile_c_size);
                    }
                });
    ...
    for (size_t tile_row = 0; tile_row < rows; tile_row += TILE_SIZE) {
        for (size_t tile_col = 0; tile_col < cols; tile_col += TILE_SIZE) {
            ...
            workflow.compute(output_bank, [=, this]() {
                ...
                orchestrator->read(input_bank, 0, src_tile.data(), tile_size);
                ...
                orchestrator->write(output_bank, 0, dst_tile.data(), tile_size);
            });
...
void EDDOMatrixOrchestrator::orchestrate_convolution(
    ...
    for (size_t ch = 0; ch < channels; ++ch) {
        ...
        workflow.compute(output_bank, [=, this]() {
            ...
            orchestrator->write(output_bank, 0, output_data.data(), output_channel_size);
        });
...
EDDOMatrixOrchestrator::OrchestrationMetrics EDDOMatrixOrchestrator::get_metrics() const {
    ...
    if (orchestrator) {
        ...
        metrics.total_operations_completed = orchestrator_metrics.completed_eddo_commands;
